...
def calc_difference2(df, G, G_row1, G_row2):
    """
    =G39-MIN(G44,75%*G39)
    """
    g39 = df.loc[G_row1, G]  # G39 in Excel, adjusting for 0-indexing in Python
    g44 = df.loc[G_row2, G]  # G44 in Excel, adjusting for 0-indexing in Python
...
def calc_custom_sum(investment_trading_securities_df, other_items_df):
    ...
    sum_ifs_result = investment_trading_securities_df.loc[investment_trading_securities_df['Rating CD'] == "L1", 'HQLA Asset'].sum()

    other_items_result = other_items_df.loc[10, 'AMOUNT ALL MATURITY'] + other_items_df.loc[11, 'AMOUNT ALL MATURITY']
...
def calculate_sum(investment_trading_securities_df, other_items_df):
    ...
    sum_ifs_result = investment_trading_securities_df[investment_trading_securities_df['Rating CD'] == "L1"]['Weight'].sum()

    other_items_result = other_items_df.at[10, 'WEIGHTS'] + other_items_df.at[11, 'WEIGHTS']
...
def main(path):
   ...
   #------------------------------------------------Insertion-------------------------------------------------------------
    ...
    #Row index 29
    output_df.at[29,'Blank 2'] = calc_difference2(output_df, 'Blank 2', 26, 21)
    output_df.at[30, 'Blank 2'] = 0
    return output_df
    #--END!
