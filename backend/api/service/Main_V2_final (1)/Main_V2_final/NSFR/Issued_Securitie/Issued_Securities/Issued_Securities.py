...
def lcr_Issued_Securities(path, input_date_str):
    ...
    iss_filename = f"Issued Securities_{input_date_str}.csv"
    reporting_date = input_date.date()
    df1 = pd.read_csv(os.path.join(path, 'input', iss_filename))
...
final_output_df = lcr_Issued_Securities(path, "28-08-2023")
