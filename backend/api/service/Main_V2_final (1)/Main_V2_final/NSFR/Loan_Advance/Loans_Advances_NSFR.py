...
def Loans_Advances_NSFR(path, input_date_str):
    loan = f"Loans & Advances_{input_date_str}.csv"
    coll = f"Collateral for Loans_{input_date_str}.csv"
    df1 =  pd.read_csv(os.path.join(path, 'Loans_Advances', 'input', loan))
    ...
    df4 =  pd.read_csv(os.path.join(path, 'Collateral_for_Loans', 'input', coll))
...
final_output_df = Loans_Advances_NSFR(path, "28-08-2023")
