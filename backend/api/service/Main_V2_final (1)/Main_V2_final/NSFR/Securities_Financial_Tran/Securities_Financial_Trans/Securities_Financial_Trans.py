...
def lcr_securitiesfinancialtrans(path, input_date_str):
    ...
    sec_filename = f"Securities Financial Trans_{input_date_str}.csv"
    reporting_date = input_date.date()

    df1 = pd.read_csv(os.path.join(path, 'input', sec_filename))
...
final_output_df = lcr_securitiesfinancialtrans(path, "28-08-2023")
