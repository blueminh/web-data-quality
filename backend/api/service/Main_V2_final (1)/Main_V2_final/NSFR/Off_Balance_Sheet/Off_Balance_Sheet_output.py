...
def lcr_offbalancesheet(path, input_date_str):
    input_date = datetime.strptime(input_date_str, "%d-%m-%Y")
    off_filename = f"Off Balance Sheet_{input_date_str}.csv"
    reporting_date = input_date.date()

    df1 = pd.read_csv(os.path.join(path,'input', off_filename))
...
final_output_df = lcr_offbalancesheet(path, "28-08-2023")
