...
def lcr_investmentandtradingsecurities(path, input_date_str):
    ...
    in_filename = f"Investment&Trading Securities_{input_date_str}.csv"
    reporting_date = input_date.date()
    df1 = pd.read_csv(os.path.join(path, 'input', in_filename))
...
final_output_df = lcr_investmentandtradingsecurities(path, "28-08-2023")
