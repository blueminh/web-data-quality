...
def Deposits_LCR(path, input_date_str):
    ...
    deposit = f"Deposit_{input_date_str}.csv"
    loan = f"Loans & Advances_{input_date_str}.csv"
    
    df1 = pd.read_csv(os.path.join(path, 'input', deposit))
    ...
    df7 = pd.read_csv(os.path.join(path, 'input', loan))
...
final_output_df = Deposits_LCR(path, "28-08-2023")
