...
def NSFR_Deposits_with_Other_Banks(path, input_date_str):
    dep = f"Deposits with Other Banks_{input_date_str}.csv"
    df1 = pd.read_csv(os.path.join(path,'Deposits_with_other_banks', 'input', dep))
    df2 = Deposits_with_other_banks_output.lcr_deposits(os.path.join(path, 'Deposits_with_other_banks'))
...
final_output_df = NSFR_Deposits_with_Other_Banks(path, "28-08-2023")
